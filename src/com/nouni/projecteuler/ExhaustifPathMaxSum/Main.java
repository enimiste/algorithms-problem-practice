...
public class Main {
	...
	private static int[] pathsMaxSums(int[][] triangle) {
		if(triangle == null || triangle.length == 0) return new int[] {};
		else if(triangle.length == 1) return new int[] {triangle[0][0]};
		else if(triangle.length == 2) return new int[] {triangle[0][0] + triangle[1][0], triangle[0][0] + triangle[1][1]};
		else {
