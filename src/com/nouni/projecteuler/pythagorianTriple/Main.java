...
public class Main {
    ...
    static R foundPytha(int sumABC) {
        for(int a = 1; a <= sumABC; a++) {
            for(int b = a + 1; b <= sumABC; b++) {
                int c = sumABC - a - b;
                if(R.check(a, b, c)) return R.as(a, b, c);
            }   
        }
    ...
    static class R {
        ...
        static R as(int x, int y, int z) {
            if(y <= x || y >= z) throw new RuntimeException(String.format("a < b < c. a=%d, b=%d, c=%d given.", x, y, z));
            if(x*x + y*y != z*z) throw new RuntimeException("a^2 + b^2 = c^2");
            R r = new R();
            r.a = x;
            r.b = y;
            r.c = z;
